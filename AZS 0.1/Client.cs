...
namespace AZS_0._1
{
    public partial class Client : Form
    {
        ...
        public void Load_data(int zp)
        {
            ...
            if (zp == 0)
            {
                a = "Select * from [Клиент]";
            }
            if (zp == 1)
            {
                a = "Select * from [Клиент] Where " + toolStripComboBox1.Text + " = '" + toolStripTextBox2.Text + "'";
            }
        ...
        {
            string a = "Delete from [Клиент] Where ID_клиента = '" + dataGridView1[0, str].Value + "'";
            using (connection = new SqlConnection(Znach.connetionString))
                try
                {
                    connection.Open();
