...
namespace AZS_0._1
{
    public partial class THH : Form
    {
        ...
        public void Load_data(int zp)
        {
            ...
            if (zp == 0)
            {
                a = "Select [Товарно-транспортная накладная].ID_ТНН, Поставщик.Название, Заправка.Адрес, Топливо.Название, [Товарно-транспортная накладная].[Количество топлива], [Цена за единицу], Сумма, [Дата получения] From [Товарно-транспортная накладная] join Заправка on[Товарно-транспортная накладная].ID_заправки = Заправка.ID_заправки join Поставщик on[Товарно-транспортная накладная].ID_поставщика = Поставщик.ID_поставщика join Топливо on[Товарно-транспортная накладная].ID_топлива = Топливо.ID_топлива";
            }
            if (zp == 1)
            {
